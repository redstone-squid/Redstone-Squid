...
class SchematicAnalyzer(Protocol):
    ...
    async def analyze(
        self, data: bytes, *, limits: SchematicLimits, with_lattice: bool = False
    ) -> SchematicAnalysis: ...
...
class SchematicStore(Protocol):
    ...
    async def record_analysis(
        self, build_id: int, sha256: str, analysis: SchematicAnalysis, *, primary: bool
    ) -> int: ...
    ...
    async def find_metric_neighbours(
        self, metrics: SchematicMetrics, *, tolerance: float, limit: int = 25
    ) -> list[StoredSchematic]:
