...
class RecordCog[BotT: "squid.bot.app.RedstoneSquid"](Cog):
    ...
    @app_commands.describe(
        ...
        kind="Optionally rebuild just doors or extenders.",
    )
    async def rebuild(
        self,
        ctx: Context[BotT],
        ...
        await ctx.send(
            view=info_layout(
                "Records rebuilt",
                f"{summary.definitions} definitions; {summary.resolved} resolved; "
                f"{summary.unresolved} awaiting evidence.",
            ),
            ephemeral=ctx.interaction is not None,
            allowed_mentions=no_mentions(),
        )
    ...
    @app_commands.describe(
        ...
        base_key="Canonical base key shown by search or records tooling.",
        restrictions="Comma-separated restriction IDs; large exact categories are saved for reuse.",
        version_id="Optional pinned version database ID.",
    )
    async def lookup(
        self,
        ctx: Context[BotT],
        ...
        try:
            ...
            msg = "Restrictions must be comma-separated numeric IDs."
        ...
        await ctx.send(
            view=info_layout(
                "Record category materialized",
                f"Recomputed {summary.definitions} definitions; {summary.resolved} resolved.",
            ),
            allowed_mentions=no_mentions(),
        )
