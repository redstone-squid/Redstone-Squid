...
class VoteSessionRepository:
    ...
    async def create_vote_session(self, kind: str, author_id: int, pass_threshold: int, fail_threshold: int, build_id: int | None = None, target_message_id: int | None = None, target_channel_id: int | None = None, target_server_id: int | None = None) -> int: ...
    ...
    async def associate_message_with_session(self, message_id: int, vote_session_id: int, purpose: str, build_id: int | None = None) -> None: ...

class VoteSessionService:
    """Service for VoteSession domain logic and orchestration (build and delete_log kinds)."""

    async def start_build_vote_session(self, author_id: int, messages: Iterable[Any], build: Any, type: Literal["add", "update"], pass_threshold: int = 3, fail_threshold: int = -3) -> int: ...
    async def start_delete_log_vote_session(self, author_id: int, messages: Iterable[Any], target_message: Any, pass_threshold: int = 3, fail_threshold: int = -3) -> int: ...
    async def close_vote_session(self, vote_session_id: int) -> None: ...
