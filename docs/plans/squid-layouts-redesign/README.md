...
| 30 | [Structural patterns (Batch B)](30-structures.md) | A shared `CommitPolicy`, MultiChoicePanel immediate commit, `Editor` (sections of forms + preview + commit policy) that SettingsPanel and the poll composer consume, and `Browser`/`Lookup` as resource-backed `WindowSource` components per 33. No new pagination abstraction — the survey validated the existing one. |
