...
| 26 | [Topic bus](26-topic-bus.md) | Portable payload-free `TopicBus` (hashable topics, sync publish, coalescing with a stated delivery contract, supervised drain) plus `sl.discord.follow` glue and a hardened `Reactor`; the expiry answer is a pre-expiry paused-status flush for token-bound mounts. Consumers are live in-process mounts only — posted messages stay with the durable `PostReconciler` — fed from the bot's two existing change chokepoints. |
