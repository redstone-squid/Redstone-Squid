...
| 40 | [Shared store](40-shared-store.md) | View state whose lifetime exceeds one mount and whose writes still belong to the action that made them: `Atom`/`Scope`/`SharedStore`/`SharedState`, staged into the existing transaction, published as addresses on 26's bus, restorable by 28's history under revision guards. Overturns 90's store rejection for that case only — no dispatch, reducers, middleware, singleton, or second domain truth. Opens by fixing `transaction()`'s unrolled-back commit path. |
