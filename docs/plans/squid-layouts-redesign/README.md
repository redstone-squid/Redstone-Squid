...
| 26 | [Topic bus](26-topic-bus.md) | Portable payload-free `TopicBus` (tuple topics, sync publish, supervised drain) with mount glue via `Reactor`; plus the expiry answer — a pre-expiry paused-chrome flush for token-bound mounts with live bindings, and the plan-23 channel-refetch experiment. First consumer: build showcase refresh. |
