...
| 11 | [API, auth, records, and sync](11-api-auth-records-sync.md) | Not started | Re-audited at `e60dded7` and re-specified with a disposition per thread. The decisive fact for its rename half: `master` carries no versioned API, so renames need no compatibility aliases. Nothing has landed yet — API-key crypto, the Discord member-resolution adapter, `discord_sync_queue`, and the route-level build policy are all as the plan describes. Its verification-code finding was handed to plan 2. |
