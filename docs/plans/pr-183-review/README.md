...
As of `d2341c80` (2026-08-17). Each plan's own Findings section already audits current HEAD, so this
...
| # | Plan | Status | Notes |
|---|---|---|---|
| 1 | [Consent and verification UX](01-consent-verification-ux.md) | Done | All five subplans landed in `c4f30e83`…`d2341c80`; see the plan's implementation notes. A verification code can now be held, so the consent prompt is a card naming the Minecraft account, the credit at stake and the receipt it will write, and a wrong code fails before the notice rather than after it. Linking renders through the same reconciliation vocabulary as refreshing, which makes the contested branch visible on link for the first time. `AliasAlreadyClaimedError` names the holding creator publicly while keeping the internal account ID in the log-only context. The hold is keyed on an opaque token because keying it on an account would have made the notice's cancellation promise false. |
| 2 | [User identity and persistence](02-user-identity-persistence.md) | Done | Subplans 1–5 were already in place. Subplan 6 landed in `8498da21`: the verification code is a keyed HMAC, ten digits (~33 bits) minted by `generate_verification_code` in the application layer rather than a bootstrap lambda, and `verification_attempts` caps consecutive failures per `(provider, subject)`. Keyed on the external identity rather than an account because the guesser may not have one yet, which also lets plan 1's anonymous reservation share the guard; holding a *correct* code never counts as a failure, or replaying your own code would lock out an already-linked user. |
