...
| # | Plan | Status | Notes |
|---|---|---|---|
| 1 | [Consent and verification UX](01-consent-verification-ux.md) | Not started | Re-audited and rewritten at `f55acda5`. `squid/bot/consent.py` is untouched dense prose and `AliasAlreadyClaimedError` still carries only `{"name": name}`. Two earlier findings were corrected: plan 02 §5 already landed batched claimant loading and `_claimant`, so only the approve/reject confirmations and the `alias_claims_pending` autocomplete are still ID-only; and the requested preview turned out to be impossible without a schema change, because the prompt runs before the code is redeemed and the only code path consumes it. The plan now leads with a two-step reserve/commit code exchange, which also gives plan 02 §6's attempt cap something countable to attach to. |
