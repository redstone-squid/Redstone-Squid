...
| 9 | [Voting redesign](09-voting-redesign.md) | Mostly done | Subplans 1–4 (domain typing, transport-independent polls, the `/poll` UI, session cleanup) re-verified fully landed — the presentation layer was generalized further than planned, into a shared reconciler/renderer. Subplan 5 is genuinely partial: fixture/builder extraction landed (`tests/helpers/voting.py`, `tests/helpers/schema.py`), but the described cross-product test matrix across `VoteKind`/`VoteTarget`/`VoteVisibility`/`VoteChoice`/`VoteRejection` was never started — only scattered single-axis parametrization exists. |
