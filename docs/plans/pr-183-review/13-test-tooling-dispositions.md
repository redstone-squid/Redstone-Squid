...
| 3775316974 | `integration/voting/.../test_vote_repository.py` | "having SQL in tests is bad" | **Fixed.** 80 lines of hand-written DDL replaced by the model metadata, via `tests/helpers/schema.py`. The account and guild foreign keys the copy had dropped are now real. Read-back `SELECT`s stay: they are the assertion, not a second setup path. |
...
| 3779995719 | `unit/bot/test_reactions.py` | "we probably need helpers" | **Fixed.** `make_reaction_payload` and `make_reaction_bot` moved to `tests/helpers/discord.py`; the payload is a real `RawReactionActionEvent` and the five `arg-type` suppressions collapsed to one cast in the helper. |
...
| 3783707542 | `unit/bot/test_errors.py` | "we really need centralized utils for discord mocks" | **Already fixed.** `test_errors.py` uses `tests/helpers/discord.py`; this plan extended that module rather than starting a second one. |
