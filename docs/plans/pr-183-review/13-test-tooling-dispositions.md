...
| 3771060757 | `unit/bot/submission/test_attachments.py` | "actually we need to verify this claim" | **Fixed.** The test no longer claims how Discord clients behave. It pins only the repository-owned input contract: missing and generic content types are both accepted when a supported schematic extension identifies the format. |
...
| 3779992042 | `unit/bot/test_reactions.py` | "subclass protocol" | **Fixed, then superseded.** The original test double subclassed `ReactionSubscriber` with `@override` methods. Plan 10 removed that mandatory four-method protocol: tests now register typed per-event bound callbacks and exercise the detachable `ReactionSubscription`, so unsupported reaction kinds need no fake methods. |
