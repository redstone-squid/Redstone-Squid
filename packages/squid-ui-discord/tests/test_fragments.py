...
from squid_ui_discord import ExistingLayoutError, ResourceCost, contribute
from squid_ui_discord import testing as sd
from squid_ui_discord.fragments import FragmentOwnershipError, StaleReservationError, fragment
from squid_ui_discord.inspection import audit
...
class TestContribute:
    def test_places_the_fragment_then_the_trailing_items(self):
        host = sd.layout_view(discord.ui.TextDisplay("header"))
        controls = sd.action_row(discord.ui.Button(label="ok", custom_id="ok"))
    ...
    def test_the_validated_view_is_the_final_view(self):
        ...
        host = sd.layout_view(discord.ui.TextDisplay("h" * 1000))
        controls = sd.action_row(*(discord.ui.Button(label="b", custom_id=f"b{index}") for index in range(5)))
    ...
    def test_degradation_is_reported_through(self):
        host = sd.layout_view(discord.ui.TextDisplay("h" * 3000))
...
class TestPreflight:
    def test_an_already_invalid_host_is_refused_before_planning(self):
        host = sd.layout_view(discord.ui.TextDisplay("x" * (LIMITS.total_text + 1)))
        with pytest.raises(ExistingLayoutError):
            contribute(_text("body"), to=host)

    def test_duplicate_custom_ids_fail_before_mutation(self):
        host = sd.layout_view(sd.action_row(discord.ui.Button(label="a", custom_id="dup")))
        before = list(host.children)
        with pytest.raises(ExistingLayoutError, match="already used"):
            contribute(
                _text("body"), to=host, followed_by=(sd.action_row(discord.ui.Button(label="b", custom_id="dup")),)
            )
    ...
    def test_an_item_owned_by_another_view_is_refused(self):
        ...
        with pytest.raises(FragmentOwnershipError, match="another view"):
            contribute(_text("body"), to=sd.layout_view(), followed_by=(trailing,))
...
class TestInteractionBoundary:
    def test_routed_controls_are_allowed(self):
        host = sd.layout_view()
        routed = squid_ui_discord.renderer.RoutedItem(label="go", custom_id="r:go")
        contribute(_text("body"), to=host, followed_by=(sd.action_row(routed),))
        assert routed in list(host.walk_children())

    def test_a_dispatchable_native_item_is_refused(self):
        native = squid_ui_discord.target.NativeItem(
            lambda: sd.action_row(discord.ui.Button(label="local", custom_id="local")),
            fallback=sl.primitives.Text("fallback"),
        )
        with pytest.raises(FragmentOwnershipError, match="does not own"):
            fragment([native], alongside=sd.layout_view())
    ...
    def test_a_native_display_item_is_fine(self):
        ...
        planned = fragment([native], alongside=sd.layout_view())
...
class TestAttachedFragment:
    ...
    def test_attaching_twice_is_refused(self):
        planned = fragment(_text("body"), alongside=sd.layout_view())
        planned.attach(sd.layout_view())
