...
@pytest.mark.parametrize(
    ...
    [
        (Scope.USER, Opener(7, 42), squid_ui_discord.SessionKey.user("panel", 7)),
        (Scope.GUILD, Opener(7, 42), squid_ui_discord.SessionKey.guild("panel", 42)),
        (Scope.USER_GUILD, Opener(7, 42), squid_ui_discord.SessionKey.user_guild("panel", 7, 42)),
...
@pytest.mark.parametrize(
    ("build", "expected"),
    [
        (Opener.user, squid_ui_discord.sessions.UserScope(7)),
        (Opener.guild, squid_ui_discord.sessions.GuildScope(42)),
        (Opener.user_guild, squid_ui_discord.sessions.UserGuildScope(7, 42)),
        (Opener.global_, squid_ui_discord.sessions.GlobalScope()),
    ],
)
...
def test_screen_ergonomics_are_promoted_from_the_public_bundle() -> None:
    assert squid_ui_discord.Opener is Opener
    assert squid_ui_discord.Scope is Scope
    assert squid_ui_discord.ScreenOptionsResolver is ScreenOptionsResolver
    assert squid_ui_discord.Navigator is squid_ui_discord.navigation.Navigator
