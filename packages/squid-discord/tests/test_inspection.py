...
class TestReservedPlanning:
    ...
    def test_text_reservation_shrinks_the_composed_view(self):
        ...
        reserved = squid_discord.compose([body], reservation=squid_discord.ResourceCost({"display_text": 1500})).view
    ...
    def test_component_reservation_is_enforced(self):
        ...
        assert len(list(squid_discord.compose(document).view.walk_children())) == 12
        with pytest.raises(sl.errors.UnsolvableLayoutError):
            squid_discord.compose(document, reservation=squid_discord.ResourceCost({"components": 35}))

    def test_a_reserved_plan_plus_the_host_fits_the_real_budget(self):
        host = _view(discord.ui.TextDisplay("h" * 2000))
        fragment_view = squid_discord.compose([sl.primitives.Text("f" * 5000)], reservation=measure(host).cost).view
