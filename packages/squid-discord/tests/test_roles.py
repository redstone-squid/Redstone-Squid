...
def panel_for(
    *,
    ...
    panel = squid_discord.RolePanel(
        ...
        title="Server roles",
        categories=(
            squid_discord.RoleCategory(
                "colour",
                "Colour",
                (
                    squid_discord.RoleOption(101, "Red", emoji="🔴"),
                    squid_discord.RoleOption(102, "Blue", emoji="🔵"),
                ),
...
def test_models_validate_panel_wide_invariants() -> None:
    with pytest.raises(ValueError, match="positive"):
        squid_discord.RoleOption(0, "Nope")
    with pytest.raises(ValueError, match="route-safe"):
        squid_discord.RoleCategory("bad:key", "Bad", (squid_discord.RoleOption(1, "One"),))
    with pytest.raises(ValueError, match="impossible"):
        squid_discord.RoleCategory(
            "one",
            "One",
            (squid_discord.RoleOption(1, "One"),),
            cardinality=squid_discord.Cardinality(minimum=2),
        )
    ...
    with pytest.raises(ValueError, match="dedicated"):
        squid_discord.RolePanel(
            group,
            title="Roles",
            categories=(squid_discord.RoleCategory("one", "One", (squid_discord.RoleOption(1, "One"),)),),
        )
...
def test_routes_and_rendering_are_stable_and_unselected() -> None:
    panel, group = panel_for(cardinality=squid_discord.EXACTLY_ONE)
    ...
    presentation = squid_discord.render_static(panel)
...
async def test_exactly_one_button_replaces_stale_selection_and_preserves_other_roles() -> None:
    panel, _ = panel_for(cardinality=squid_discord.EXACTLY_ONE)
    roles = {
        101: FakeRole(101, 10),
        102: FakeRole(102, 11),
        303: FakeRole(303, 12),
...
async def test_invalid_cardinality_is_reported_and_different_members_do_not_share_locks() -> None:
    ...
    assert squid_discord.roles._MEMBER_LOCKS == {}
