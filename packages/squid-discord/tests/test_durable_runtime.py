...
def components() -> ComponentRegistry:
    ...
    registry.register(
        "counter",
        version=1,
        restore=lambda context: squid_discord.Mount(Counter(), access=Everyone(), timeout=None),
    )
    registry.register(
        "hidden-draft",
        version=1,
        restore=lambda context: squid_discord.Mount(HiddenDraft(), access=Everyone(), timeout=None),
    )
    return registry
...
async def test_suppressed_runtime_commit_checkpoints_hidden_component_state() -> None:
    ...
    mount = squid_discord.Mount(component, access=Everyone(), timeout=None)
...
async def test_attached_mount_is_checkpointed_in_the_same_record() -> None:
    ...
    async with anyio.create_task_group() as tasks:
        ...
        assert isinstance(opened, Opened)
        child = squid_discord.Mount(Counter(), access=Everyone(), timeout=None)
...
async def test_a_durable_quota_survives_recovery() -> None:
    ...
    async with anyio.create_task_group() as tasks:
        ...
        await tasks.start(first.run)
        opened = await first.open(
            squid_discord.Mount(Counter(), access=Everyone(), timeout=None),
            delivered_to(fake_message(message_id=99)),
            recipe="counter",
            key=key,
            actor_id=7,
