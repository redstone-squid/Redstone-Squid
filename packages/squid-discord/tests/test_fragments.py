...
from squid_discord import ExistingLayoutError, ResourceCost, contribute
from squid_discord.fragments import FragmentOwnershipError, StaleReservationError, fragment
from squid_discord.inspection import audit
...
class TestInteractionBoundary:
    def test_routed_controls_are_allowed(self):
        host = _host()
        routed = squid_discord.renderer.RoutedItem(label="go", custom_id="r:go")
    ...
    def test_a_dispatchable_native_item_is_refused(self):
        native = squid_discord.target.NativeItem(
...
def test_a_fragment_never_exceeds_whole_document_planning():
    ...
    body = "x" * 6000
    whole = squid_discord.compose([sl.primitives.Text("header"), sl.primitives.Text(body)]).view
