...
async def live_subject(**kwargs: Any) -> Mount:
    ...
    await mount.send(delivered_to(squid_discord.testing.fake_message(message_id=42)))
    return mount
...
class TestDetail:
    ...
    async def test_a_detail_view_distinguishes_an_armed_dirty_application(self) -> None:
        ...
        await subject.send(
            delivered_to(
                squid_discord.testing.fake_message(message_id=42, ephemeral=True),
                handle=squid_discord.delivery.handle_from(interaction),
            )
        )
    ...
    async def test_a_registry_key_labels_its_mount(self) -> None:
        registry = squid_discord.SessionRegistry()
        subject = Mount(Subject(), access=Everyone())
        await registry.open(subject, delivered_to(squid_discord.testing.fake_message()), key=("editor", 7))
