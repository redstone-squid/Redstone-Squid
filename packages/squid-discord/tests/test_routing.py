...
class TestRouter:
    ...
    async def test_a_select_handler_receives_values_then_route_parameters(self) -> None:
        ...
        await router.dispatch(
            fake_interaction(),
            "edit:build:42",
            component=squid_discord.routing.RouteComponent.SELECT,
            values=("one", "two"),
    ...
    def test_describe_returns_stable_public_route_metadata(self) -> None:
        ...
        assert router.describe() == (
            squid_discord.routing.RouteDescription(
                component=squid_discord.routing.RouteComponent.SELECT,
...
class TestRouteGroups:
    async def test_child_groups_build_stable_routes_and_dispatch_them(self) -> None:
        root = squid_discord.routing.RouteGroup("r")
    ...
    def test_a_namespace_group_is_an_ordinary_one_segment_group(self) -> None:
        nested = squid_discord.routing.RouteGroup("r", "polls")
    ...
    def test_group_prefixes_are_nonempty_literal_segments(self, prefix: tuple[str, ...]) -> None:
        with pytest.raises(ValueError, match=r"route group|literal segment"):
            squid_discord.routing.RouteGroup(*prefix)
    ...
    def test_a_group_from_another_namespace_is_rejected(self) -> None:
        ...
        foreign = squid_discord.routing.RouteGroup("other")
...
class TestMiddleware:
    def test_the_base_class_requires_dispatch(self) -> None:
        with pytest.raises(TypeError, match="abstract"):
            cast(Any, squid_discord.routing.Middleware)()

    async def test_router_and_group_middleware_compose_outermost_first(self) -> None:
        seen: list[str] = []

        class Record(squid_discord.routing.Middleware[discord.Client]):
        ...
        root = squid_discord.routing.RouteGroup[discord.Client]("r")
        polls = root.group("polls")
    ...
    async def test_returning_without_calling_next_short_circuits_and_still_acknowledges(self) -> None:
        ...
        class Stop(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                seen.append("stopped")
    ...
    async def test_middleware_may_handle_an_inner_exception(self) -> None:
        ...
        class Catch(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                try:
                    await proceed()
                except RuntimeError:
    ...
    async def test_unhandled_errors_cross_the_whole_onion_before_the_error_hook(self) -> None:
        ...
        class Observe(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                seen.append("before")
                try:
                    await proceed()
    ...
    async def test_proceed_is_one_shot(self) -> None:
        ...
        class Twice(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                await proceed()
                await proceed()
    ...
    async def test_proceed_expires_when_middleware_returns(self) -> None:
        saved: list[squid_discord.routing.RouteProceed] = []

        class Save(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
    ...
    async def test_request_facts_are_immutable_and_distinguish_aliases(self) -> None:
        requests: list[squid_discord.routing.RouteRequest[discord.Client]] = []

        class Capture(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
    ...
    def test_descriptions_include_effective_middleware_provenance(self) -> None:
        class RouterPolicy(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                await proceed()

        class GroupPolicy(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                await proceed()
    ...
    def test_middleware_freezes_at_registration(self) -> None:
        class Policy(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                await proceed()
...
class TestProfiling:
    async def test_route_trace_profiles_middleware_handler_and_acknowledgement(self) -> None:
        class Continue(squid_discord.routing.Middleware[discord.Client]):
            async def dispatch(self, request, proceed) -> None:
                await proceed()
...
class TestClientRegistration:
    def test_installed_routers_have_a_public_read_only_snapshot(self) -> None:
        ...
        assert squid_discord.routing.routers(client) == (first, second)  # type: ignore[arg-type]
