...
class TestBudgetContract:
    ...
    def test_collectively_unsatisfied_floors_raise(self) -> None:
        ...
        target = v2(limits=V2Limits(total_text=100))
...
class TestRegionPagination:
    ...
    def test_a_section_pages_heterogeneous_children(self) -> None:
        ...
        first = sl.planning.plan(document, target=DISCORD_V2_DPY27)
        second = sl.planning.plan(
            document, target=DISCORD_V2_DPY27, positions={"report": sl.sources.Position(offset=1)}
        )
    ...
    def test_keep_with_next_moves_a_heading_to_its_content(self) -> None:
        ...
        first = sl.planning.plan(document, target=DISCORD_V2_DPY27)
        second = sl.planning.plan(
            document, target=DISCORD_V2_DPY27, positions={"chapters": sl.sources.Position(offset=1)}
        )
    ...
    def test_unbreakable_rejects_an_oversized_group(self) -> None:
        ...
        with pytest.raises(sl.errors.UnsolvableLayoutError, match="unbreakable region child"):
            sl.planning.plan(document, target=DISCORD_V2_DPY27)
    ...
    def test_an_oversized_text_child_splits_losslessly(self) -> None:
        ...
        pages = [
            sl.planning.plan(document, target=DISCORD_V2_DPY27, positions={"prose": sl.sources.Position(offset=index)})
    ...
    def test_widows_keep_three_children_on_the_last_page(self) -> None:
        ...
        last = sl.planning.plan(document, target=DISCORD_V2_DPY27, positions={"widows": sl.sources.Position(offset=1)})
