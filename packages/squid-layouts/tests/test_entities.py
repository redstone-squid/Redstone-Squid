...
def test_entity_reference_rejects_non_positive_ids() -> None:
    with pytest.raises(ValueError, match="positive"):
        sl.semantic.EntityRef(sl.semantic.EntityKind.USER, 0)


def test_picker_rejects_incompatible_defaults_and_channel_filters() -> None:
    with pytest.raises(ValueError, match="incompatible"):
        EntitySelect(
            sl.semantic.EntityType.USER,
            _select,
            "users",
            default_values=(sl.semantic.EntityRef(sl.semantic.EntityKind.ROLE, 1),),
        )
    with pytest.raises(ValueError, match="channel_types"):
        EntitySelect(sl.semantic.EntityType.ROLE, _select, "roles", channel_types=(sl.semantic.ChannelType.TEXT,))


def test_entity_select_costs_an_action_row_and_control() -> None:
    solved = measure([EntitySelect(sl.semantic.EntityType.USER, _select, "users")])
...
def test_native_semantic_picker_lowers_to_entity_scene() -> None:
    plan = sl.planning.plan(sl.entities(key="users", entity_type=sl.semantic.EntityType.USER), target=sl.discord.V2_TARGET)
...
def test_semantic_picker_uses_enumerated_fallback_without_capability() -> None:
    ...
    plan = sl.planning.plan(
        sl.entities(
            sl.entity_choice(sl.semantic.EntityRef(sl.semantic.EntityKind.USER, 1), "Ada"),
            key="users",
            entity_type=sl.semantic.EntityType.USER,
        ),
        target=target,
    )
...
def test_semantic_picker_refuses_without_native_capability_or_fallback() -> None:
    ...
    with pytest.raises(sl.errors.LayoutInvariantError, match="enumerated fallback"):
        sl.planning.plan(sl.entities(key="users", entity_type=sl.semantic.EntityType.USER), target=target)


def test_entity_scene_round_trips_mixed_mentionable_defaults() -> None:
    node = SceneEntitySelect(
        sl.semantic.EntityType.MENTIONABLE,
        "mentions",
        default_values=(
            sl.semantic.EntityRef(sl.semantic.EntityKind.USER, 1),
            sl.semantic.EntityRef(sl.semantic.EntityKind.ROLE, 2),
        ),
    )
    plan = sl.planning.plan(
        EntitySelect(
