...
def test_agreement_renders_display_names_chrome_and_only_ephemeral_state() -> None:
    ...
    mount = sl.discord.Mount(agreement, access=sl.discord.Users({1, 2}), timeout=None)
...
async def test_frontend_neutral_membership_check_rejects_an_outsider() -> None:
    ...
    mount = sl.discord.Mount(agreement, access=sl.discord.Everyone(), timeout=None)
    commit_render(mount)
...
def test_agreement_validates_identity_threshold_and_controls() -> None:
    ...
    mount = sl.discord.Mount(agreement, access=sl.discord.Users({1}), timeout=None)
