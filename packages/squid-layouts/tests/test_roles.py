    ...
    panel = sl.discord.RolePanel(
        ...
        categories=(
            sl.discord.RoleCategory(
                ...
                "Colour",
                (
                    sl.discord.RoleOption(101, "Red", emoji="🔴"),
                    sl.discord.RoleOption(102, "Blue", emoji="🔵"),
                ),
...
def test_models_validate_panel_wide_invariants() -> None:
    with pytest.raises(ValueError, match="positive"):
        sl.discord.RoleOption(0, "Nope")
    with pytest.raises(ValueError, match="route-safe"):
        sl.discord.RoleCategory("bad:key", "Bad", (sl.discord.RoleOption(1, "One"),))
    with pytest.raises(ValueError, match="impossible"):
        sl.discord.RoleCategory(
            "one",
            "One",
            (sl.discord.RoleOption(1, "One"),),
            cardinality=sl.discord.Cardinality(minimum=2),
        )
    ...
    with pytest.raises(ValueError, match="dedicated"):
        sl.discord.RolePanel(
            group,
            title="Roles",
            categories=(sl.discord.RoleCategory("one", "One", (sl.discord.RoleOption(1, "One"),)),),
        )
...
def test_routes_and_rendering_are_stable_and_unselected() -> None:
    panel, group = panel_for(cardinality=sl.discord.EXACTLY_ONE)
...
async def test_exactly_one_button_replaces_stale_selection_and_preserves_other_roles() -> None:
    panel, _ = panel_for(cardinality=sl.discord.EXACTLY_ONE)
    roles = {
        101: FakeRole(101, 10),
        102: FakeRole(102, 11),
        303: FakeRole(303, 12),
...
async def test_invalid_cardinality_is_reported_and_different_members_do_not_share_locks() -> None:
    ...
    assert sl.discord.roles._MEMBER_LOCKS == {}
