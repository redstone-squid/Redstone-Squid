...
def test_inert_tally_renders_counts_bars_and_reader_emphasis() -> None:
    result = sl.planning.plan(sl.tally(OPTIONS, key="vote"), target=sl.discord.V2_TARGET)
...
async def test_mounted_tally_inherits_button_adaptation_and_reports_one_key() -> None:
    ...
    result = sl.planning.plan(sl.tally(OPTIONS, key="vote", on_vote=vote), target=sl.discord.V2_TARGET)
    button = next(
        item
