...
def test_zoned_datetime_retains_instant_and_named_zone() -> None:
    value = sl.ZonedDateTime(datetime(2026, 8, 22, 14, 30, 5, 123456, tzinfo=UTC), "America/New_York")
...
def test_zoned_datetime_normalizes_aware_instant_to_utc() -> None:
    value = sl.ZonedDateTime(datetime.fromisoformat("2026-08-22T10:30:00-04:00"), "America/New_York")
...
def test_zoned_datetime_identity_includes_timezone() -> None:
    ...
    assert sl.ZonedDateTime(instant, "America/New_York") != sl.ZonedDateTime(instant, "America/Toronto")


def test_zoned_datetime_rejects_naive_instants_and_unknown_zones() -> None:
    with pytest.raises(ValueError, match="aware"):
        sl.ZonedDateTime(datetime(2026, 8, 22, 14, 30), "UTC")  # noqa: DTZ001 - exercises naive rejection
    with pytest.raises(ValueError, match="unknown IANA timezone"):
        sl.ZonedDateTime(datetime(2026, 8, 22, 14, 30, tzinfo=UTC), "Mars/Olympus_Mons")
