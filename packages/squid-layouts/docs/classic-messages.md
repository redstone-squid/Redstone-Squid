...
from squid_layouts.discord import CLASSIC_TARGET, V2_TARGET, classic
...
`sl.discord.render_static` becomes a `LayoutView` instead. Nothing in the document changed.
...
```python
host = sl.discord.DiscordPresentation.classic(view=DoorView())
...
mount = sl.discord.Mount(Door(), target=CLASSIC_TARGET, access=sl.discord.Owner(user_id))
await mount.send(sl.discord.reply_to(ctx))
```
...
handler genuinely needs the interaction, reach for it explicitly with `sl.discord.native(event)`
...
```python
mount = sl.discord.Mount(Door(), target=V2_TARGET, access=sl.discord.Owner(user_id))
```
...
length, row and control structure, and attachment capacity. `sl.discord.audit_classic_payload`
...
```python
from squid_layouts.discord import Target, TargetRegistry
...
mount = components.restore(snapshot, targets=targets, access=sl.discord.Everyone())
```
