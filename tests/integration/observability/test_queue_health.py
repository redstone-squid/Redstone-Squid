...
async def _health(session_factory: async_sessionmaker[AsyncSession]) -> dict[str, object]:
    async with session_factory() as session:
        return {row.queue: row for row in (await session.execute(QUEUE_HEALTH_STATEMENT)).mappings().all()}
...
async def test_a_claim_moves_a_queue_from_ready_to_in_flight(
    ...
    assert (before["ready"], before["in_flight"]) == (1, 0)  # pyright: ignore[reportIndexIssue]
    ...
    assert (after["ready"], after["in_flight"]) == (0, 1)  # pyright: ignore[reportIndexIssue]
...
async def test_a_backed_off_row_ages_from_when_it_was_enqueued(
    ...
    assert (row["ready"], row["in_flight"], row["dead_letters"]) == (0, 0, 0)  # pyright: ignore[reportIndexIssue]
...
async def test_a_queue_without_dead_lettering_reports_a_constant_zero(
    ...
    assert row["dead_letters"] == 0  # pyright: ignore[reportIndexIssue]
