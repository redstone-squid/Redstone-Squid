...
from squid_reactive import LocalTopicBus, Reactive, Shared, state, transaction


def _shared_type(cells: int) -> type[Shared[str]]:
    ...
    return type(f"BenchmarkShared{cells}", (Shared,), namespace)
...
def core_baseline() -> dict[str, dict[str, int]]:
    """Measure publishing transactions over 0/1/10/100 strongly read Shared cells."""
    results: dict[str, dict[str, int]] = {}
    for cells in (0, 1, 10, 100):
        model = _shared_type(cells)(LocalTopicBus(), "benchmark")

        def publish(model: Shared[str] = model, cells: int = cells) -> None:
            with transaction():
